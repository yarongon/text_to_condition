...
import pytest
from src.text_to_condition.exporters import (
...
def test_human_readable_exporter():
    ...
    assert exporter.export(and_condition) == "amount greater than or equal to 100 AND category is food"
    ...
    assert exporter.export(or_condition) == "amount less than 50 OR category is transport"
    assert exporter.export(in_condition) == "category.isin([food, transport])"
